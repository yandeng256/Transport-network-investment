...
# To run the code, define the ivtSet and region
...
# In[]: Running economic loss for all return period, and find the annual expected loss and repair cost
baseline = baseCost 
disUC=[]    
for i in range(10):
    start = time.clock()    
...
summary = np.column_stack((rpTime,isolationT,remainI,lossTripI,isoLoss,\
                           ...
                           np.array(disruptC)+np.array(isoLoss),lossTrip,V,\
                                   (np.array(disruptC)+np.array(isoLoss))/V,\
