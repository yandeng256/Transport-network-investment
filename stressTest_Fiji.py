...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan  3 13:38:56 2018
...
# In[]:
# this file is used for stress testing. The only different with ecoLoss_Fiji is the last segment:stress testing

# before investment, ivtSet= {}
# after investment, ivtSet comes from optimization file, or any given set of investment. 
# To run the code, define the ivtSet, region and the simulation iteration for stress testing.
# output: stressTest: the result of stress testing;  stressPara: the parameters as ratio to baseline value for repair cost, duration and travel demand

# investment set, change this one when you change investment option
# we have north_1,north_2,north_3; south_1,south_2, south_3; those are given government list
# north_opt; south_opt; those are generate by optimization script
# or you may define your new investment structure list

ivtSet = {} 
region = 'north'
#region = 'south' 
numCases = 3 # number of simulation iteration
# In[]: investment data
#: Given investment set
...
# investment set from optimization script    
north_opt ={102, 119, 145, 605, 657, 670, 676, 679, 680, 683, 723, 724, 732, 811, 1112, 1118, 1127, 1231} #opt
south_opt={3, 4, 22, 46,216,217,296,329,444,448,458,479,768,769,835,1010,1093,1094,1200,1841,2278} #from PU optimal
...
# In[]: find the shortest path in the initial network before disaster. 
...
# baseline: find the shortest path for each od to minimize the total travel cost; 
# output: 1) baseLength: total travel distance between all OD pairs; this is the input of OD matrix calculation
...
for i in range(len(od)):
    for j in range(i+1,len(od)):
        T[i][j]= k*popu[od[i]]*popu[od[j]]/baseLength[i][j]/30*1e3/50 # AADT (passenger per day)
...
# In[]: Calculate the transport service loss due to disaster disruption or isolation for each OD pair
# model proposed by Stephane
...
# In[]: Update the road network graph given whether a link is disrupted or not

def updateGraph(G, stru_damage, link_damage, item, linkBreak):
    stru_damage.add(item) # save the structure ID if it's disrupted for that return period
    
    if not dic_strLink[item] in link_damage:
        link_damage.append(dic_strLink[item]) # save the link ID if it's disrupted for that return period
    
    if linkBreak:
        G[dic_strNode[item][0]][dic_strNode[item][1]]['total_cost']=1e10 # the link is completely disrupted
    else:
        w = G[dic_strNode[item][0]][dic_strNode[item][1]]['total_cost']
        G[dic_strNode[item][0]][dic_strNode[item][1]]['total_cost']=2*w # the travel cost double because of the Bailey bridge
    
# In[]: compute the repair cost for a structure
# Input: r: return period; RC, maximum repair cost- bridge $13000,culvert $2500,crossing $1250 per unit area. (data from Julie field trip)
# stdLevel: disruption criteria. For exmaple, bridge 50 return period at low priority  
# item: structure OBJECTID    
def computeRepairCost(r, RC, repairC, stdLevel, item):
    i = dic_strID[item] # from structure OBJECTID to row index
    l = float(df_structure['Length'][i]) 
    if np.isnan(l): # fill the missing information
        l=10
    b = float(df_structure['TotalWidth'][i] )
    if np.isnan(b):
        b=3
            
    # fully demange of infrastructure if extra proportion is greater than 1 (rules suggested by Xavier)
...
# In[]: compute the economic user cost under an updated graph. 
...
def computeNewGraphCost(dic_graph, stru_name, G, baseline, iso, days, isoTrip_sum, num, T, disLoss, demandA_B):
    ...
    # single source dijkstra shortest path algorithm, to accelerate the computational speed
    ...
    for j in range(len(to_allNode)):
        for k in range(len(od)):
            if k>j:
               cost_disrupt[j][k] = to_allNode[j].get(od[k]) # find the travel cost for each OD pair under disaster

    for index, item in np.ndenumerate(cost_disrupt): # if OD isolated, here we seperate the calculation of disruption and isolation.  
        if item>=1e10:
            cost_disrupt[index]=baseline[index]
            iso[index] += T[index] * days / 1e6               
            isoTrip_sum[loc] += T[index] * days / 1e6   # total isolation trips(million) for a return period. It has three durations
    ...
    for j in range(len(od)): # for each OD pair 
        for k in range(j+1,len(od)):
            C_0 = baseCost[j][k] # initial cost
            N_0 = T[j][k]
            C_1 = cost_disrupt[j][k] # cost increment because of disruption
            if N_0 !=0:
    ...
    # In[]:  
    # The main function of user disruption cost computation under a certain return period
    ...
    # Input:
    #       r: return period
    #       graph: original transportation network
    #       curIvtSet: Structure ID under investment
    #       baseline: Original travel cost at OD matrix 
    #       demand: original demand at OD matrix
    #       sensitivity factor, equals to 1 at baseline: bridgeRC_r, culvertRC_r, crossingRC_r, bridge_r, culvert_r, crossing_r,dema_r
...
def disrupt(r,graph,curIvtSet,baseline,bridgeRC_r, culvertRC_r, crossingRC_r, bridge_r, \
    ...
    # three durations: we calculate the economic loss seperately for each duration
    bridgeT= 330*bridge_r # duration 3, after a month up to the rest of the year, crossing, culvert recovered, travel cost double for the bailey bridge links 
    culvertT = 24*culvert_r # duration 2, after first week up to the first month after disaster, crossing recovered, culvert and bailey bridge under repair 
    crossingT = 7*crossing_r # duration 1, the first week after disasater, all structures are disrupted
    ...
    repairC2 = [] # (culvert)
    repairC3 = [] # (crossing)
    dic_graph = {} # the disrupted graph in each duration
    ...
    for link, strList in dic_linkStr.items(): # classfied the structures to three categories
        for item in strList:
...
# in economic loss calculation, we begin from duration 3, where only bridge are under repair; 
# then duration 2, where both bridges and culverts under repair, then duration 1, all structures are under repair   
# We make it this order because we want to add the damaged links to the graph cumulatively, from less to more and more             

###################################################################  duration 3: using bailey bridge, double travel cost 
    for item in bridge_set:
        ...
        elif item not in curIvtSet: # the damage criteria is proposed by Julie
...
############################################################## repair culvert and building bailey bridge
    for item in bridge_set:
        ...
        if item in curIvtSet and df_structure[r][i]>df_structure['DPU17_100'][i]: # invest but disrupt, assume after invest, design standard=100 yr
            ...
            repairC1.append(1e6) # repair cost of installing bailey bridge, cost 1 million dollar (from Julie)
    ...
    ####################################  economic loss due to isolation

    boatCost = baseline*10 # if the OD is isolation, we assume the travel cost is 10 times the original the travel cost for alternative travel mode
    ecoLoss = np.zeros((len(od),len(od)))
    demandAfter = np.zeros((len(od),len(od)))
    demandBefore = np.zeros((len(od),len(od))) 
...
# In[]: calculate the total consumer surplus, V, million$ per year
...
st.to_csv('Fiji_streeTest.csv', sep=',') 
stressPara.to_csv('Fiji_streePara.csv', sep=',') 
